...
class DAFormerHead(BaseDecodeHead):
    ...
    def __init__(self, **kwargs):
        ...
        for i, in_channels, embed_dim in zip(self.in_index, self.in_channels,
                                             embed_dims):   
            # [0,1,2,3],[64,128,320,512],[256,256,256,256]
            # 这里的embed_cfg和embed_neck_cfg其实是一样的，都是mlp（也就是1x1卷积）
    ...
    def forward(self, inputs):
        ...
        # 先上采样到同一个大小H/4,W/4,四个256
        # 然后再fuse，通过aspp网络融合到1024xH/4xW/4的
        # 再通过一个1x1卷积到num_class维度
        # 注：这里的图片是resize之后的，只有512x512
        # 所以这里是bs x 1024 x 128 x 128
