...
class LesionDataset(CustomDataset):
    ...
    def _evaluate(self, results, metric='mIoU', evaluate_per_image=False, logger=None, **kwargs):
        ...
        #========================================================
        iou, f1, ppv, s, aupr , ae = lesion_metrics(
        ...
        for i in range(num_classes):
            ...
            #========================================================
            ae_str = '{:.2f}'.format(ae[i]*100)
            summary_str += line_format.format(class_names[i], iou_str, f1_str, ppv_str, s_str, aupr_str,ae_str)
        ...
        mMAE = np.nanmean(np.nan_to_num(ae[-4:],nan=0))

        summary_str += 'Summary:\n'
        line_format = '{:<15} {:>10} {:>10} {:>10} {:>10} {:>10} {:>10} {:>10}\n'
        summary_str += line_format.format('Scope', 'mIoU', 'mF1', 'mPPV', 'mS', 'mAUPR','mMAE(e-2)')
        ...
        ae_str = '{:.2f}'.format(mMAE*100)
        summary_str += line_format.format('global', iou_str, f1_str, ppv_str, s_str, aupr_str,ae_str)
