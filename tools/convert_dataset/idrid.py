...
'''
Please, first rename the original directories of IDRiD dataset to:
...
def parse_args():
    parser = argparse.ArgumentParser(description='Reconstruct the IDRiD dataset')
    parser.add_argument('--root', default='./data/IDRiD')
    ...
    for subset in os.listdir(image_root):
        for eachimage in os.listdir(os.path.join(image_root,subset)):
            ...
            for each_class in CLASSES:
                each_class_gt_path = os.path.join(label_root,subset,each_class,eachimage.replace('.jpg',f'_{each_class}.tif'))
                each_class_gt = cv2.imread(each_class_gt_path,cv2.IMREAD_GRAYSCALE)
                fused_gt[each_class_gt==76] = CLASSES[each_class]
